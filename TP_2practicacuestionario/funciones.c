...
int VerNoDocentes(){
    ...
    int noDocentes=0;
    long long suma=0;
    unsigned int promedio=0,diferenciaMenor=99999999,diferencia=0,dniCercano;
    ...
    while(fread(&usuario, sizeof(usuario_t), 1, archivo)){
        tipo_usuario = usuario.TipoStatus >> 4;
        estado = usuario.TipoStatus & 0b00001111;
        if(tipo_usuario==NoDocente&&estado==Expirado){  
    ...
    if(noDocentes!=0){
        printf("Se encontraron %d no docentes\n\n",noDocentes);
        
        printf("El promedio de DNI de no docentes expirados es de: %d\n\n",promedio);
        printf("%s, DNI: %d\n\n", nombreCercano, dniCercano);
        return funciono;
    }
    else{
