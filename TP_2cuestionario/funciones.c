...
int VerNoDocentesExpirados(){
    ...
    char nombre_cercano[50];
    unsigned int DNI_cercano=0,diferencia=0,menor_diferencia=99999999;
    archivo=fopen("credenciales.bin", "rb"); 
    if (archivo == NULL) { 
        perror("Error al abrir el archivo");
        return fallo;
    ...
    if(estudiantes_existen!=0){
        promedio=(unsigned int)suma/estudiantes_existen;
        printf("Se encontraron %d NoDocentesExpirados\n\n",estudiantes_existen);
        printf("El promedio del DNI de no docentes expirados es: %lu\n\n",promedio);
        while (fread(&usuario, sizeof(usuario_t), 1, archivo)) {
            tipo_usuario = usuario.tipo_usuario_estado >> 4;
            estado = usuario.tipo_usuario_estado & 0b00001111;
            if(tipo_usuario==no_docente&&estado==expirado){
                if(usuario.DNI<promedio){
                    diferencia=promedio-usuario.DNI;
                }
                else
                    diferencia=usuario.DNI-promedio;
                if(menor_diferencia>=diferencia){
                    menor_diferencia=diferencia;
                    DNI_cercano=usuario.DNI;
                    strcpy(nombre_cercano,usuario.nombre);
                }
            }
        }

        printf("El DNI mas cercano es: %u y pertenece al usuario: %s\n\n",DNI_cercano,nombre_cercano);
        
        return funciono;
    }
    else{
        printf("No se encontraron docentes en el archivo\n\n");
        return fallo;
    }
